...
const CreateOrEditArticle = () => import('./views/article/CreateOrEditArticle.vue')
...
const routes = [{
        ...
        children: [{
                ...
                beforeEnter: (to, from, next) => {
                    ...
                    next(store.dispatch(TYPES.article.actions.GET_ARTICLES, null, {
                        root: true
                    }));
                }
            },
...
const requestIfNotInitial = (arr) => {
    arr.forEach(item => {
        if (!store.state[item].status) {
            switch (item) {
                case 'tag':
                    store.dispatch(TYPES.tag.actions.GET_TAGS, null, {
                        root: true
                    });
                    break;
                case 'category':
                    store.dispatch(TYPES.category.actions.GET_CATEGORIES, null, {
                        root: true
                    });
                    break;
                case 'config':
                    store.dispatch(TYPES.config.actions.GET_CONFIG, null, {
                        root: true
                    });
                    break;
            }
...
router.beforeEach((to, from, next) => {
    ...
    if (to.path !== '/login') {
        if (user == null) {
            ...
            store.commit(TYPES.user.mutations.UPDATE_USER, {
                status: true,
                user: user
            });
            next();
