...
export default {
	...
	computed: {
		...mapState({
			categories: state => {
				return Object.values(state.category.categories);
			},
	...
	methods: {
		...
		handle(data) {
			...
			if (this.operation === 'create') {
				this.$store
					.dispatch(TYPES.category.actions.CREATE_CATEGORY, { category: params }, { root: true })
					.then(() => {
				...
				this.$store
					.dispatch(TYPES.category.actions.UPDATE_CATEGORY, { id: id, category: params }, { root: true })
					.then(() => {
						this.dialogVisible = false;
						if (!this.status) this.$message.error(this.msg);
					});
