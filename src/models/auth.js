...
const auth = {
    ...
    actions:{
        async login({ commit },payload){
            ...
            if(result.success){
                if(result.res.type==='admin'){
                    commit(TYPES.user.mutations.UPDATE_USER,{user:result.res,status:result.success},{root:true});
                    commit(LOGIN_SUCCESS);
                    token.setAuth(result.res.token);
                    router.push('/');
                }else{
                    commit(LOGIN_FAIL,{msg:'The user has no access!'})
                }
            }else
        ...
        async logout({commit},payload){
            ...
            if(result.success){
                commit(TYPES.user.mutations.UPDATE_USER,{status:false,user:{}},{root:true});
                token.clearAuth();
                router.push('/login')
            }
        }
