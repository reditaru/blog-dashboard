...
const category = {
    ...
    actions:{
        async getCategories({commit}){
            ...
            if(data.success){
                commit(TYPES.category.mutations.UPDATE_ALL_CATEGORY,{categories:data.res},{root:true});
                commit(TYPES.category.mutations.OPERATION_SUCCESS,null,{root:true});
            }
        },
        async deleteCategory({commit},payload){
            commit(TYPES.category.mutations.OPERATION_REQUEST,null,{root:true});
            let data = await categoryService.deleteCategory(payload);
            if(data.success){
                commit(TYPES.category.mutations.DELETE_CATEGORY,{id:payload.id},{root:true})
                commit(TYPES.category.mutations.OPERATION_SUCCESS,null,{root:true});
            }else{
                commit(TYPES.category.mutations.OPERATION_FAIL,{msg:data.msg},{root:true});
            }
        },
        async createCategory({commit},payload){
            ...
            if(data.success){
                commit(TYPES.category.mutations.UPDATE_CATEGORY,{category:data.res},{root:true})
                commit(TYPES.category.mutations.OPERATION_SUCCESS,null,{root:true});
            }else{
                commit(TYPES.category.mutations.OPERATION_FAIL,{msg:data.msg},{root:true});
            }
    ...
    mutations:{
        [UPDATE_ALL_CATEGORY](state,payload){
            let categories = {};
            payload.categories.forEach(item=>{categories[item.id]=item})
