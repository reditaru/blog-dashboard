...
const tag = {
    ...
    actions:{
        async getTags({commit}){
            ...
            if(data.success){
                commit(TYPES.tag.mutations.UPDATE_ALL_TAG,{tags:data.res},{root:true});
                commit(TYPES.tag.mutations.OPERATION_SUCCESS,null,{root:true});
            }
        },
        async deleteTag({commit},payload){
            commit(TYPES.tag.mutations.OPERATION_REQUEST,null,{root:true});
            let data = await tagService.deleteTag(payload);
            if(data.success){
                commit(TYPES.tag.mutations.DELETE_TAG,{id:payload.id},{root:true})
                commit(TYPES.tag.mutations.OPERATION_SUCCESS,null,{root:true});
            }else{
                commit(TYPES.tag.mutations.OPERATION_FAIL,{msg:data.msg},{root:true});
            }
        },
        async createTag({commit},payload){
            ...
            if(data.success){
                commit(TYPES.tag.mutations.UPDATE_TAG,{tag:data.res},{root:true})
                commit(TYPES.tag.mutations.OPERATION_SUCCESS,null,{root:true});
            }else{
                commit(TYPES.tag.mutations.OPERATION_FAIL,{msg:data.msg},{root:true});
            }
    ...
    mutations:{
        [UPDATE_ALL_TAG](state,payload){
            let tags = {};
            payload.tags.forEach(item=>{tags[item.id]=item})
            state.tags = tags
        },
